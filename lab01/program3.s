...
#   x5  → sentinel value indicating the end of the list
############################################################
...
forfl2:                    # Start of the loop for flag 2
    ...
    beq x26, x5, forfl3     # If current byte == sentinel, go to flag 3 check
...
forfl3a:                    # Start of the loop for flag 3
    lb x26, 0(x27)             # Load current byte into x26
    addi x27, x27, 1           # Move pointer to next element
    beq x26, x5, End           # If current byte == sentinel, jump to End
